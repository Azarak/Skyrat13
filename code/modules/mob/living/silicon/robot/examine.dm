	...
	switch(stat)
		if(CONSCIOUS)
			if(shell)
				...
				. += "<span class='warning'>It appears to be in stand-by mode and has been completely unresponsive to anything for [round(((world.time - lastclienttime) / (1 MINUTES)),1)] minutes. It may boot up soon.</span>" //SKYRAT CHANGE - ssd stuff
		if(UNCONSCIOUS)
